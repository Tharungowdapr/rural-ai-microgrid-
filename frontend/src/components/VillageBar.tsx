    ...
    return (
        <div className="flex gap-3 overflow-x-auto pb-1">
            {villages.map((v) => {
                ...
                return (
                    <div key={v.id}
                        className="flex-none w-[240px] bg-zinc-900/90 border border-zinc-800 rounded-lg p-3">
                        <div className="flex items-center justify-between mb-2">
                            <span className="text-sm font-bold text-zinc-100">{v.name}</span>
                            <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold border ${STATUS_BG[v.status] || 'bg-zinc-700 text-zinc-300'}`}>
                                {v.status}
                            </span>
                        </div>
